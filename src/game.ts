...
export class Game {
    ...
    constructor(canvasId: string) {
        ...
        this.offsetX = 0;
        this.offsetY = 0;
        this.trafficManager = new TrafficManager(this.cityMap, this.renderer, this.MAP_WIDTH, this.MAP_HEIGHT);
    ...
    private runSimulationStep(): void {
        ...
        this.trafficManager.cycleTrafficLights();
    ...
    private renderTile(x: number, y: number, tile: any, gridX: number, gridY: number): void {
        ...
        switch (tile.type) {
            ...
            case TileType.ROAD:
                ...
                if (roundaboutTopLeft) {
                    ...
                    const roadVehicles = this.trafficManager.getVehicles().filter(v => v.tileX === gridX && v.tileY === gridY);
        ...
        if (this.showTrafficDensity && tile.type === TileType.ROAD) {
            this.trafficManager.drawTrafficDensityOverlay(ctx, x, y, tile.traffic);
        }
    ...
    private startGameLoop(): void {
        const loop = (currentTime: number) => {
            ...
            // Delta time an TrafficManager weitergeben
            this.trafficManager.setDeltaTime(this.deltaTime);
            
            // Fahrzeuge in jedem Frame bewegen (nicht nur bei Simulation)
            this.trafficManager.updateVehicles((x, y) => this.getRoadConnections(x, y));
